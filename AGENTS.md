...
本仓库是一个金融舆情监测 POC。前端应用位于 `web/`，使用 React/Vite；爬虫与 API 后端位于 `server/`，使用 Express/TypeScript；共享 API 契约位于 `openapi/openapi.yaml`；初始化数据与运行期 JSON 数据位于 `data/`。
...
- `server/` 是后端：Node.js ESM、Express 5、TypeScript、爬虫适配器、NLP 服务、JSON 存储、去重服务和温度计算服务。
- `data/` 存放前后端都会使用的 JSON 持久化/配置数据。将这些文件视为应用数据，只有在任务确实需要调整种子数据或配置时才修改。
...
- 在明确要求进行 JSON 到数据库迁移之前，不要引入生产数据库假设。
